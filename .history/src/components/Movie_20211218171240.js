...
function Movie({id,converImg, title, summary, genres}) {    
    ...
    <div className={styles.movie}>
        ...
        <h2>
            <Link className={styles.title} to ={`/movie/${id}`}>{title.length > 30 ? `${title.slice(0, 30)}...` : title}</Link>
        </h2>
        <p className={styles.summary}>{summary.length > 230 ? `${summary.split(0, 250)}...` : summary}</p>
        <ul>
          {genres.map(g => 
